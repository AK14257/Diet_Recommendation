...
class PerticularResturnatMenuScreen extends StatefulWidget {
  const PerticularResturnatMenuScreen(
      {super.key, required this.resturantUID, required this.ResturantName});
  final String resturantUID;
  final String ResturantName;

  @override
  State<PerticularResturnatMenuScreen> createState() =>
      _PerticularResturnatMenuScreenState();
}

class _PerticularResturnatMenuScreenState
    extends State<PerticularResturnatMenuScreen> {
  void initState() {
    super.initState();
    WidgetsBinding.instance.addPostFrameCallback((_) {
      context.read<ResturantProvider>().emptyResturantMenu();
      context.read<ResturantProvider>().getResturantMenu(widget.resturantUID);
    });
  }
  ...
  Widget build(BuildContext context) {
    return SafeArea(
      ...
      body: Consumer<ResturantProvider>(
        ...
        if (resturantProvider.resturantMenu.isEmpty) {
          return Center(
            child: Text(
              'Fetching Food Items',
          ...
          return ListView.builder(
              itemCount: resturantProvider.resturantMenu.length,
              shrinkWrap: true,
              padding: EdgeInsets.symmetric(horizontal: 3.w, vertical: 2.h),
              physics: const BouncingScrollPhysics(),
              itemBuilder: (context, index) {
                FoodModel foodData = resturantProvider.resturantMenu[index];
                return InkWell(
                  onTap: () {
                    Navigator.push(
                      context,
